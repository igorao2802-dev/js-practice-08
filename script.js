...
function loadPreferences() {
    try {
        return {
            perPage: localStorage.getItem(PREF_PER_PAGE_KEY) || "5",
            sortBy: localStorage.getItem(PREF_SORT_BY_KEY) || "updated_desc",
        };
    } catch (error) {
        console.warn("Ошибка загрузки настроек:", error);
        return { perPage: "5", sortBy: "updated_desc" };
    }
}

function savePreferences(perPage, sortBy) {
    try {
        localStorage.setItem(PREF_PER_PAGE_KEY, perPage);
...
function renderRepos(repos, append = false) {
    ...
    repos.forEach((repo) => {
        ...
        updated.title = `Последнее обновление: ${updateDate.toLocaleDateString("ru-RU", {
            day: "2-digit",
            month: "long",
            year: "numeric",
        })}`;
...
function renderPagination(totalPages) {
    ...
    if (appState.windowStart > 1) {
        ...
        prevBtn.innerHTML = "← Назад";
        prevBtn.addEventListener("click", () => {
            appState.windowStart = Math.max(1, appState.windowStart - PAGES_PER_WINDOW);
...
perPageSelect.addEventListener("change", (event) => {
    ...
    savePreferences(appState.perPage, sortBySelect ? sortBySelect.value : "updated_desc");
    updateReposDisplay();
});
...
if (sortBySelect) {
    sortBySelect.addEventListener("change", () => {
        ...
        appState.windowStart = 1;
        // 💾 Сохраняем настройки при изменении
        savePreferences(appState.perPage, sortBySelect.value);
        fetchAndRenderRepos();
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    appState.perPage = prefs.perPage;
    perPageSelect.value = prefs.perPage; // Синхронизируем UI
    
    appState.windowStart = 1;
    initScrollToTop();
    
    if (sortBySelect) {
        sortBySelect.value = prefs.sortBy; // Применяем сохранённую сортировку
    }
});
